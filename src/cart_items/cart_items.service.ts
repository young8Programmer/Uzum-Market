...
export class StoreOwnersService {
  ...
  async create(createCartItemDto: CreateCartItemDto): Promise<CartItem> {
    ...
    if (!user) {
        throw new NotFoundException(`ID ${createCartItemDto.user_id} bo'lgan foydalanuvchi topilmadi`);
    }
    ...
    if (existingItem) {
        throw new ConflictException("Ushbu mahsulot ushbu foydalanuvchi uchun allaqachon qo'shilgan");
    }
  ...
  async findOne(id: number): Promise<CartItem> {
    const storeOwner = await this.storeOwnersRepository.findOne({ where: { id } });
    if (!storeOwner) {
  ...
  async update(id: number, UpdateCartItemDto: UpdateCartItemDto): Promise<CartItem> {
    ...
    if (UpdateCartItemDto.user_id) {
      ...
      if (!user) {
        throw new NotFoundException(`ID ${UpdateCartItemDto.user_id} bo'lgan foydalanuvchi topilmadi`);
      }
    }
